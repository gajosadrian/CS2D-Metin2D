...
declare namespace string {
   ...
   * ends; otherwise, it returns nil. A third, optional numeric argument init
   ...
   * pattern (see §6.4.1) have been replaced by a replacement string specified by
   ...
   * kept in the string).
   */
  function gsub(s: string, pattern: string, repl: string | Function, n?: number): string;
   ...
   */
  function match(s: string, pattern: string, init?: unknown): string;
   ...
   * by the string sep. The default value for sep is the empty string (that is, no
   * separator). Returns the empty string if n is not positive.
   *
   ...
   * same as the string length). In particular, the call string.sub(s,1,j) returns
   * a prefix of s with length j, and string.sub(s, -i) (for a positive i) returns
   * a suffix of s with length i.
   *
   * If, after the translation of negative indices, i is less than 1, it is corrected
   * to 1. If j is greater than the string length, it is corrected to that length.
   * If, after these corrections, i is greater than j, the function returns the empty
   * string.
   */
  function sub(s: string, i: number, j?: number): string;
  /**
   * Returns the values packed in string s (see string.pack) according to the format
   * string fmt (see §6.4.2). An optional pos marks where to start reading in s

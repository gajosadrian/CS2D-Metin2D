...
function Chunk:getIndexesFromCollection(channel, where)
    ...
    for i = 1, #collection do
        ...
        if type(where) == 'string' then
            if data.__CHUNK_key ~= where then
                valid = false
            end
        elseif where then
...
function Map:new(width, height)
    ...
    if MapInstance then
        print('\169249001001LUA ERROR: ChunksMap instance already exists')
        return
    end
    MapInstance = o
...
function Map:initEntity(channel, obj, x, y)
    ...
    obj.__CHUNK_chunkId = chunk.id
    obj.__CHUNK_key = tostring(obj)
    obj.__CHUNK_channel = channel
end

function Map:updateEntity(obj, x, y)
    local new_chunk = self:getChunkAt(x, y)

    if obj.__CHUNK_chunkId ~= new_chunk.id then
        local chunk = self:getChunkById(obj.__CHUNK_chunkId)
        chunk:changeCollectionChunk(new_chunk, obj.__CHUNK_channel, obj.__CHUNK_key)
        obj.__CHUNK_chunkId = new_chunk.id
    end
end
...
function Map:getChunkById(chunk_id)
    return self.chunks[chunk_id] or nil
end

function Map:getChunkIdAt(x, y)
    if not self.tiles[x] or not self.tiles[x][y] then return nil end
    return self.tiles[x][y]
end

function Map:getChunkAt(x, y)
    local chunk_id = self:getChunkIdAt(x, y)
    if not chunk_id then return nil end
...
function Map:removeFromCollection(chunk_id, channel, where, neighbours)
    self:getChunkById(chunk_id):removeFromCollection(channel, where, neighbours)
end

function Map:getFromCollection(chunk_id, channel, where, neighbours)
    local chunk = self:getChunkById(chunk_id)
    if not chunk then return nil end
    chunk:getFromCollection(channel, where, neighbours)
end
